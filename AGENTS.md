...
6. When writing or changing tests, compare the intended observation: resolve actual and expected filesystem locations before containment checks; capture Python console reports whose contract is text with `text=True, encoding="utf-8"`; preserve source, protocol and artifact bytes with byte reads and writes. If path spelling or stream bytes are the contract, assert them unchanged. Reproduce path aliases and CRLF through real files and processes on the development host, and report native-platform results separately.
