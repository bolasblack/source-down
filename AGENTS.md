...
1. Before changing behavior, specifications, or tests, read the [specification index](docs/specs/README.md), the [model](docs/specs/model.md), and the owning specification. For a new requirement, write the clause first, then add one failing test through a public entry point, implement the smallest passing change, and refactor while green.
