...
标准调用的 directive 第一版恰好为 `include`；arguments 是 SPEC-DIR-006 的封闭对象，
...
已求值的有序 Markdown 块替换该指令行，参与 [渲染规则](rendering.md) 定义的块间编排，
...
include 的全部参数、选择与呈现语义由 [内置内容条款](standard-directives.md) 拥有。
...
插件按 kind 的 UTF-8 字节序、path 字节序、recursive（false 在前）排序去重；核心接纳任意数组顺序并归一化。每轮结果按插件归属保留依赖集合，供使用方取得；首版不执行监听、变化比较或跨轮结果缓存。显式 file 依赖（包括无内容来源或尚不存在者）参与 [SPEC-CLI-007](cli.md#spec-cli-007) 的发布目标保护；目录依赖不禁止其整个子树中的输出。
