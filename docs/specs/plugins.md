...
超时、取消、I/O、协议或语义校验故障均终止并回收会话内全部受管进程组或等价进程作用域。不得自动重试本轮；执行故障后不再发送后续业务批次。watch 观察到修复事实后可建立新的 Session，遵循 [SPEC-CLI-010](cli.md#spec-cli-010) 和 [SPEC-CLI-011](cli.md#spec-cli-011)。清理必须覆盖直接子进程退出后仍持有管道的后代；SIGINT 遵循 [SPEC-CLI-005](cli.md#spec-cli-005)。

已退出的直接子进程仍须回收。终止信号返回权限错误后，若回收直接子进程并确认整个受管进程组已经不存在，不报告清理失败；仅有直接子进程退出不能证明后代已经清理。仍存在的受管进程无法终止时，保留实际清理错误。
...
项目插件须声明影响自身计算的项目内脚本、导入模块及其他材料；watch 按外部 owner 的依赖变化重建 Session，遵循 [SPEC-CLI-011](cli.md#spec-cli-011)。
