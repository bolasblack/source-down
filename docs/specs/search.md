...
内联替换后的完整叶子正文与各个内联调用的终值分别收录；紧凑列表中的省略 paragraph 事件的正文同样属于叶子块。
内联展开的 occurrence 紧随所属正文，缺少可检索外层正文时仍保留展开项；原文 mapping 只包含实际保留的作者字节。
...
目录事实描述成功发布后的状态：构建时对所见成员应用本轮确定的父目录创建、产物写入、旧报告删除和 watch 明确拥有的旧页面删除。
