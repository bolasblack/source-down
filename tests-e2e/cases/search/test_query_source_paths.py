...
class QuerySourcePaths(SelfUseCase):
    specs = ('SPEC-SRH-003', 'SPEC-SRH-004', 'SPEC-SRH-005', 'SPEC-SRH-006')

    def test_scenario(self):
        """查询 来源 路径 标点 在两个输出根命中并能按句柄准确读回"""
        declarations = self.declaredSearchQueries()
        self.assertEqual(len(declarations), 5)
        self.assertEqual(declarations[2], {'query': '来源 路径 标点', 'path': 'docs/guide', 'kind': 'prose', 'top_k': 3, 'expected_input': 'docs/guide/building-pages.md', 'reason': 'The chapter explains source paths and literal punctuation.'})
            ...
            for outputDir in (".source-down", "custom-reading"):
                ...
                with self.subTest(output=outputDir):
                    found = project.sourceDown.search(
                        '来源 路径 标点', path='docs/guide', limit=3,
                        outputDir=outputDir,
                    )
                    self.assertSearchResult(found, exitCode=0, freshness="matched")
                    hit = self.assertMatchingHit(
                        found, kind='prose', inputPath='docs/guide/building-pages.md',
                    )
