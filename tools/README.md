...
- `spec_alignment.py` assembles one review packet per spec clause and maintains the alignment ledger. A round lives under `$XDG_CACHE_HOME/source-down/alignment/` (or `~/.cache/...`) as one folder with `packets/` and `spec-alignment-ledger.md`. `--new-run` starts a round with an empty ledger; a default run continues the latest round, prints `spec_alignment: run PATH`, then lists stale, unreviewed and orphaned clauses and exits 1 when any list has an entry. `--record CLAUSE --verdict V [--note TEXT]` rebuilds that one packet in the selected round and rewrites that ledger; `--forget CLAUSE` drops one row and does nothing else. `--archive DIR` copies a round; that is the keep step. `mise run alignment` renders the project first; pass `-- --new-run` to start a round. It reaches Source Down only through the public CLI (`--binary` defaults to the release build) and opens nothing else under the product output root. See the [spec alignment contract](../docs/engineering/spec-alignment.md).
- `test.py` runs native Rust, Python and readable E2E through the bounded process scheduler in `test_jobs.py`, collects fresh profiles, generates HTML/JSON reports, and enforces separate 90% production line coverage gates. `--jobs N` controls the shared budget; `--no-coverage` uses the same collections without instrumentation; `--review` publishes E2E reading from this execution. Release uses `--artifact PATH --spec-plugin PATH` for parallel E2E and portability against exact supplied files. `test_suite.py` owns discovery and assembly, `acceptance_worker.py` owns one preserved E2E module, and `process_scope.py` owns child cleanup. `coverage.toml` configures Python subprocess measurement; see the [coverage guide](../docs/engineering/coverage.md).
